...
{
    AtomicBitfield( size_t m_size )
        : m_size(m_size)
        , state( ceil_div(m_size, bitfield_len) )
    {
...
private:
    ...
    inline std::optional< T > probe_chunk_by_value( unsigned j, uint64_t mask, bool expected_value, F && f )
    {
        while( true )
        {
            uint64_t field = expected_value ?
                uint64_t(state[j]) : ~uint64_t(state[j]);
