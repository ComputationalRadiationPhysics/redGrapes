...
#    define REDGRAPES_EMPLACE_NOTIFY_NEXT 0
            ...
            auto id = SingletonContext::get().worker_pool->probe_worker_by_state<unsigned>(
                [](unsigned idx)
                {
        ...
        void DefaultScheduler::activate_task(Task& task)
        {
            ...
            static thread_local std::atomic<unsigned int> next_worker(
                SingletonContext::get().current_worker ? SingletonContext::get().current_worker->get_worker_id() + 1
                                                       : 0);
        ...
        Task* DefaultScheduler::steal_task(dispatch::thread::Worker& worker)
        {
            ...
            if(Task* task = steal_new_task(worker))
            {
                ...
                {
                    SingletonContext::get().worker_pool->set_worker_state(
                        worker_id,
                        dispatch::thread::WorkerState::BUSY);
                    return task;
                }
